...
contract STARS is Context, IERC20, Ownable {
    ...
    address payable public marketingWallet = payable(0x8D87558Aa3a112474157054c08ee326A6F89df45);
    ...
    uint256 public swapTokensAtAmount = 200_000_000_000_000 * 10**9; // 1/5000 * total supply
    uint256 public swapTokensAtMaxAmount = 1_000_000_000_000_000 * 10**9; // 1/1000 * total supply
    ...
    if (enableAntiwhale && maxTransferAmount() > 0 && sender != uniswapV2Pair) {
        if (
    ...
    function _transfer(
        ...
        if (
            ...
            if (contractTokenBalance > swapTokensAtMaxAmount){
                contractTokenBalance = swapTokensAtMaxAmount;
            }
                

            uint256 liquidityTokenShare = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_marketingFee));
            uint256 marketingTokenShare = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee));
      
            //add liquidity and send bnb to wallets

            if(liquidityTokenShare > 0){
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        ...
        require(taxFee+liquidityFee+marketingFee <= 25, "max 25%");
    }
    
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner() {
        swapTokensAtAmount = newAmount*10**9;
    }
    function setSwapTokensAtMaxAmount(uint256 newAmount) external onlyOwner() {
        swapTokensAtMaxAmount = newAmount*10**9;
    }
